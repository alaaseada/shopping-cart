...
const products_wrapper = document.querySelector('#products-wrapper');
...
async function loadData() {
  if (!isPostBack) {
    ...
    json_data.forEach((item) => {
      const item_index = itemsInCart.findIndex((elem) => elem.id == item.id);
      const link_span =
        item_index >= 0
          ? `<span class="cart-span" id="span-${item.id}">In cart</span>`
          : `<a href="#" class="cart-link" id=${item.id}><i class="fa-solid fa-cart-shopping"></i> Add to cart</a>`;
      products_wrapper.innerHTML += `<div class="product-cell">
              <div class="img">
...
function showOverlay(e) {
  if (e.target.classList.contains('product-cell')) {
...
function handleCartSpan(id) {
  ...
  link.href = '#';
  link.innerHTML = `<i class="fa-solid fa-cart-shopping"></i> Add to cart`;
  if (span) {
...
function showCart(e) {
  cart_side_panel.className = 'visible-cart';
  overlay_div.className = 'visible';
  fillCart();
}

function hideCart(e) {
  cart_side_panel.className = 'hidden-cart';
...
products_wrapper.addEventListener('click', addToCart);
products_wrapper.addEventListener('mouseenter', showOverlay);
products_wrapper.addEventListener('mouseleave', hideOverlay);
